    ...
    tablist = {
        newTab: function (item) {
            ...
            if (flag) {
                ...
                $.hzsofttab.scrollToTab($('.menuTab.active'));
            }
        }
    }
    $.hzsofttab = {
        ...
        scrollTabRight: function () {
            ...
            var tabOuterWidth = $.hzsofttab.calSumWidth($(".content-tabs").children().not(".menuTabs"));
            ...
            if ($(".page-tabs-content").width() < visibleWidth) {
                ...
                scrollVal = $.hzsofttab.calSumWidth($(tabElement).prevAll());
                if (scrollVal > 0) {
                    $('.page-tabs-content').animate({
        ...
        scrollTabLeft: function () {
            ...
            if ($(".page-tabs-content").width() < visibleWidth) {
                ...
                offsetVal = 0;
                if ($.hzsofttab.calSumWidth($(tabElement).prevAll()) > visibleWidth) {
                    while ((offsetVal + $(tabElement).outerWidth(true)) < (visibleWidth) && tabElement.length > 0) {
                        offsetVal += $(tabElement).outerWidth(true);
        ...
        scrollToTab: function (element) {
            var marginLeftVal = $.hzsofttab.calSumWidth($(element).prevAll()), marginRightVal = $.hzsofttab.calSumWidth($(element).nextAll());
            var tabOuterWidth = $.hzsofttab.calSumWidth($(".content-tabs").children().not(".menuTabs"));
            var visibleWidth = $(".content-tabs").outerWidth(true) - tabOuterWidth;
            var scrollVal = 0;
        ...
        init: function () {
            $('.menuItem').on('click', top.$.hzsofttab.addTab);
            $('.menuTabs').on('click', '.menuTab i', top.$.hzsofttab.closeTab);
            $('.menuTabs').on('click', '.menuTab', top.$.hzsofttab.activeTab);
            $('.tabLeft').on('click', top.$.hzsofttab.scrollTabLeft);
            $('.tabRight').on('click', top.$.hzsofttab.scrollTabRight);
            $('.tabReload').on('click', top.$.hzsofttab.refreshTab);
            ...
            $('.tabCloseOther').on('click', $.hzsofttab.closeOtherTabs);
            $('.fullscreen').on('click', function () {
                if (!$(this).attr('fullscreen')) {
                    $(this).attr('fullscreen', 'true');
                    $.hzsofttab.requestFullScreen();
                } else {
                    $(this).removeAttr('fullscreen')
                    $.hzsofttab.exitFullscreen();
                }
            });
        }
    };
    $.hzsoftindex = {
        ...
        loadMenu: function () {
            ...
            $ul.find("li").click(function () {
                ...
                var childNodes = $.hzsoftindex.jsonWhere(data, function (v) { return v.ParentId == dataId });
                var _childNodeshtml = "";
                $.each(childNodes, function (i) {
                    var row = childNodes[i];
                    var subchildNodes = $.hzsoftindex.jsonWhere(data, function (v) { return v.ParentId == row.ModuleId });
